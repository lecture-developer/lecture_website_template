...
class ResearchPosition extends Element
{
	...
	toHtml()
	{
		...
		'</div><hr></div><div class="team-content-section collapsing-section open-section">' + this.description + '</div></div>';
		// requierments
		if (this.requierments.length > 0)
		{
			...
			'</div><hr></div><div class="team-content-section collapsing-section open-section"><ul class="research-requierment-list">';

			this.requierments.forEach(requierment => {
				html += '<li><svg style="margin-right: 10px;" width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="4" cy="4" r="4" fill="#1A202C"/></svg>' + requierment + '</li>';
			});
		...
		if (!(this.applay_text == "" || this.connect_email == ""))
		{
			...
			'</div><hr></div><div class="team-content-section collapsing-section open-section  col-important">';
